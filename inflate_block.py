...
def main(data_dir, file_name, class_num, class_name, out_dir):
    ...
    if not os.path.exists("{0}/rotation_block_labels".format(data_dir)):
        os.makedirs("{0}/rotation_block_labels".format(data_dir))
    ...
    for i, img in enumerate(trans_img):
        ...
        if not os.path.exists("{0}/rotation_block_labels/{1}".format(data_dir, class_name)):
            os.makedirs("{0}/rotation_block_labels/{1}".format(data_dir, class_name))
        ...
        if i == 0:  # 画像変更なし
            copyfile("{0}/labels-first/{1}/{2}".format(data_dir, class_num, text_file_name),
                     "{0}/rotation_block_labels/{1}/{2}".format(data_dir, class_name, new_file_name + ".txt"))
        elif i == 1: # 横反転
            with open("{0}/rotation_block_labels/{1}/{2}".format(data_dir,
                                                           class_name,
...
if __name__ == '__main__':
    ...
    # クラス名のファイルの読み込み
    classes = []
    classes = [line.rstrip() for line in open('{0}/block.txt'.format(data_dir), 'r')]
