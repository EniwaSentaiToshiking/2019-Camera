...
def main(data_dir, file_name, class_num, class_name, out_dir):
    ...
    # 画像の読み込み
    img_src = cv2.imread(
        "{0}/rotation_block_images/{1}/{2}".format(data_dir, class_num, file_name), 1)
    trans_img = []
    ...
    for i, img in enumerate(trans_img):
        ...
        copyfile("{0}/rotation_block_labels/{1}/{2}".format(data_dir, class_num, text_file_name),
...
if __name__ == '__main__':
    ...
    for _, dirs, _ in os.walk("{0}/rotation_block_images/".format(data_dir)):
        for class_name in classes:
            for class_num in dirs:
                if class_num == class_name:
                    print("{0}/rotation_block_images/{1}".format(data_dir, class_name))
                    for _, _, files in os.walk("{0}/rotation_block_images/{1}".format(data_dir, class_num)):
